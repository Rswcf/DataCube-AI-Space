...
const META: Record<string, { title: string; description: string; ogDescription: string; ogAlt: string }> = {
  de: {
    title: `${BRAND.name} | Tägliche KI-News & Einblicke`,
    ...
    ogAlt: `${BRAND.name} – Wo KI auf menschliche Einsicht trifft`,
  },
  en: {
    ...
    ogAlt: `${BRAND.name} – Where AI meets human insight`,
  },
  zh: {
    title: `${BRAND.name} | 每日 AI 新闻与洞察`,
    description: '免费多语言AI新闻聚合：生成式AI突破、大模型动态、AI投资信号和实用技巧 – 每日从40+来源精选，支持8种语言。',
    ogDescription: '精选 AI 新闻、投资动态和实用技巧 — 每日更新，支持 8 种语言。',
    ogAlt: `${BRAND.name} – AI 与人类智慧的交汇`,
  },
  fr: {
    title: `${BRAND.name} | Actualités IA quotidiennes`,
    ...
    ogAlt: `${BRAND.name} – Où l'IA rencontre l'intelligence humaine`,
  },
  es: {
    title: `${BRAND.name} | Noticias diarias de IA`,
    ...
    ogAlt: `${BRAND.name} – Donde la IA se encuentra con la inteligencia humana`,
  },
  pt: {
    title: `${BRAND.name} | Notícias diárias de IA`,
    ...
    ogAlt: `${BRAND.name} – Onde a IA encontra a inteligência humana`,
  },
  ja: {
    title: `${BRAND.name} | 毎日のAIニュースとインサイト`,
    ...
    ogAlt: `${BRAND.name} – AIと人間の知恵が出会う場所`,
  },
  ko: {
    title: `${BRAND.name} | 매일 AI 뉴스 & 인사이트`,
    ...
    ogAlt: `${BRAND.name} – AI와 인간의 통찰력이 만나는 곳`,
  },
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const localizedHome = absoluteUrl(`/${lang}`)
  const meta = META[lang] || META.en

  const hreflangEntries: Record<string, string> = { 'x-default': BRAND.siteUrl }
