...
2. **Attribution:** the founder publishes under their real name as creator/operator ("Made by <founder>"). Content is AI-generated and labeled; the product never claims human editing ("Edited by …" forbidden). EU AI Act Art. 50(4) applies since 2026-08-02 (no human-review exception) — labeling is an obligation (confirm with counsel). **Amended 2026-09-16 (founder decision):** the byline is "Made by Deepviews", a brand the founder owns, rather than a personal name. The Impressum (DDG §5) still names the operator in person; that is an R3a entry criterion.
...
**AD7 — Honest attribution and AI labeling.** Schema.org: articles authored and published by the Organization; founder as `Organization.founder` (Person). Footer "Made by <founder>". Every surface that renders AI-written prose carries a label near the top in the page language: week/day pages, article pages, topic pages, emails (all editions), `/feed.xml`, `/newsletter.xml`, `/api/content-summary`, `llms.txt`, OG text. Person-like bylines ("Data Cube AI Editorial / Redaktion / 编辑部") are replaced. **Amended 2026-09-16 (R2):** because the configured founder is the Deepviews brand, `Organization.founder` is typed Organization, not Person (R2 Ruling R-15). The label also appears in the email preheader and on each `/feed.xml` entry, where inbox lists and feed readers show AI text first (R2 final review).
...
- **Subscribe hardening** (`app/api/subscribe/route.ts`): server-side email validation; per-IP rate limit (not the cookie/origin guard); default language `en`; `double_opt_override: "on"` with `reactivate_existing: true` (forced DOI protects consent and lets a real reader re-subscribe); `utm_source`/`referring_site`. **Amended 2026-09-15, after the R1 release:** Beehiiv reactivates an opted-out address immediately and skips double opt-in, so the route looks the address up first, never reactivates (`reactivate_existing: false`), and answers an opted-out address exactly like a new signup; readers who unsubscribed rejoin through the contact form.
