...
> **状态:日期快照(2026 年 5 月)。** 设计原则仍有效,但产品定位已于 2026-08 从团队工具转向大众产品(EN 默认);与实现相关的细节以代码为准。
...
每天有 500+ 篇 AI 相关文章发布，但大多数专业人士只读 3 篇。Data Cube AI 应运而生——一个开源 AI 新闻聚合器，扫描 RSS、Hacker News、YouTube 和 Reddit 等来源，通过 4.5 阶段 LLM 管道对内容进行分类、摘要和翻译，为用户提供 3 分钟即可读完的 8 语言 AI intelligence memo。
...
> **Data Cube AI** 是一个 8 语言 AI intelligence 平台，
> 每日 3 分钟即可掌握最重要的 AI 新闻。
> 与 Feedly AI 或 Google News 不同，
> 我们免费、开源，同时提供 DE、EN、ZH、FR、ES、PT、JA、KO 内容。
...
- "8 languages. One concise intelligence memo."
...
当前网站和 Newsletter 采用“杂志式 intelligence memo”方向：

1. **主标题** 使用 Newsreader，建立清晰的出版物气质。
2. **导航、标签、日期、分类** 使用 Geist 大写小号字，字距保持 0，不使用负字距。
3. **内容边界** 依靠细黑线、浅色分割线、序号、日期徽章和留白，而不是层层嵌套卡片。
4. **趋势列表** 使用 01/02/03 的编辑编号，点击进入 Topic Hub；编号是信息层级，不是装饰。
5. **Newsletter CTA** 可使用出版物黄色块，但正文区域保持白底、黑字、红棕色强调，避免单一蓝/紫科技感。
6. **页面结构** 左侧承载分类与时间轴，中间承载正文内容，右侧承载搜索、订阅和趋势辅助信息；不要在中间区域重复侧边栏已有导航。
...
| **多语言** | 8 语言自然、语气一致、结构平行 | 生硬翻译、语言混杂 |
...
| **实用技巧** | 标题 + 难度徽章 + 可操作的段落 |
| **Newsletter 主题行** | "[核心新闻] | Data Cube AI | [日期]"（按订阅者语言本地化）|
