...
**8 语言 AI 新闻聚合器（DE/EN/ZH/FR/ES/PT/JA/KO）**，自动策展科技突破、投资动态、实用技巧和 YouTube 视频 — 由 4.5 阶段 LLM 流水线驱动。
...
Data Cube AI 自动从 **22 个 RSS 订阅源**、**Hacker News** 和 **YouTube** 收集、分类和摘要 AI 新闻，然后以简洁的 8 语言界面呈现，支持每日和每周视图。
...
- **8 语言支持** — 内容覆盖 DE、EN、ZH、FR、ES、PT、JA、KO
...
| **对话与报告** | openrouter/free (OpenRouter) |
| **股票数据** | Polygon.io API |
| **托管** | Vercel（前端）、Railway（后端 + 数据库 + 定时任务） |
| **设计** | Newsreader 字体、等距立方体 Logo、分区配色方案、渐进式动画 |
...
| **3. 处理** | 并行 LLM 处理：生成 DE/EN 基础摘要、提取实体 | 30 条科技 + 21 条投资 + 15 条技巧 + 5 个视频 |
| **3.5. 翻译** | 将英文内容翻译为 ZH、FR、ES、PT、JA、KO，并通过模型链重试 | 每条内容增加 6 个语言版本 |
...
| `/api/trends/{periodId}` | GET | 当前时间段的趋势主题 |
...
│   │   ├── [lang]/news/         # 文章页面 (SEO/GEO)
│   │   ├── [lang]/topic/        # Topic Hub
│   │   ├── [lang]/tools/        # 本地化工具页面
